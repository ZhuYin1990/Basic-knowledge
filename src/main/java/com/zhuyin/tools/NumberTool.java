...
public class NumberTool {
	...
	public static int isDaffodiNumber(int num) {
		...
		for (int i = 0; i < n; i++) {
			...
			sum += Math.pow((int) (num / Math.pow(10, i)) % 10, 3); // jdk5以后新特性，自动拆装箱    sum=sum+double,自动转成了int
		}
		...
		if (num==1) {
			...
			while(minPrimeNumber<=num){
				...
				if (minPrimeNumber==num) {
					sb.append(num);
					break;
				}else if (num%minPrimeNumber==0) {	//如果这个输大于最小质数，但能被最小质数整除，则拼接最小质数后用n/k的值作为新的n，循环
	...
	public static int maxCommonDivisor(int num1, int num2){
		...
		if (num1%num2==0) {	//2.如果较大值能够整除较小值，那么最大公约数就是最小值，返回最小值即可
			return num2;
